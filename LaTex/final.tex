...
\subsection{Regula fasi}

The false position or Regula Fasi method is a bracketed method for finding a numerical solution to an equation of the form $ f(x = 0 $ within a given interval $ [a, b] $ .
...
For the execution of the methods we looked for different functions that allow the comparison of these methods for this we will run each of the methods with the corresponding functions, and we will use the linux time tool that allows us to measure how long it takes to run our program.
...
10         & \multicolumn{1}{c|}{0.205}  & \multicolumn{1}{c|}{0.23}  \\ \hline
\end{tabular}
...
From this, the average of these 10 executions is kept looking for the average execution time of the method, finding that the method executed with c++ is 10 times more efficient than the one made in python.
...
From this we take the average of these 10 executions looking for the average execution time of the method, finding that the method executed with c++ is more or less 20 to 30 times more efficient.
